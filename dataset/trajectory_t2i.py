...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, args, data_folder, split='train', preprocesser=None):
        ...
        self.videos = sorted(os.listdir(self.reference_path))
    ...
    def get_metadata(self, idx):
        ...
        img_name = sorted(os.listdir(os.path.join(self.data_path, "initial", video_name)))[img_idx]

        img_path = os.path.join(video_name, img_name)

        input_frame = Image.open(os.path.join(self.reference_path, img_path))
        trajectory_frame = Image.open(os.path.join(self.trajectory_path, img_path))
        target_frame = Image.open(os.path.join(self.target_path, img_path))
